...
x_train, x_test, y_train, y_test = train_test_split(x, y, train_size=0.9, shuffle=True, random_state=551)
...
model = Sequential()
model.add(Conv2D(filters=90, kernel_size=(2,2),padding='same',input_shape=(5, 6, 1)))
model.add(Conv2D(filters=90, kernel_size=(2,2),padding='same'))
model.add(Conv2D(filters=60, kernel_size=(2,2),padding='same'))
model.add(Conv2D(filters=60, kernel_size=(2,2),padding='same'))
...
# cp = ModelCheckpoint(monitor='acc', )
lr = ReduceLROnPlateau(monitor='acc', factor=0.5, mode='max', verbose=1, patience=10)
