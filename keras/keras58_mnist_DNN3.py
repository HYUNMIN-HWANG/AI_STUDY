...
# Dense 다차원 output도 가능하다. (마지막 레이어에 아웃풋 노드수를 잘 조정한다.)
...
model.add(Dense(64))
# model.add(Flatten())    # 4차원 인풋 그대로 아웃풋으로 내려가기 위해서 Flatten 지운다.
model.add(Dense(64))
model.add(Dense(1))
...
reduce_lr = ReduceLROnPlateau(monitor='val_loss', patience=5, factor=0.5, verbose=1)
                    # patience : 3만큼 기다리고
                    # factor : 개선이 없으면 learning rate를 0.5로 감축시키겠다.
                    # 예시 ) ReduceLROnPlateau reducing learning rate to 0.0005000000237487257. (default : 0.001)
