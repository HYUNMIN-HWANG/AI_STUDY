...
#3. 컴파일 & 훈련
model.compile(loss = 'mse', optimizer='adam') #머신이 이해할 수 있도록 컴파일 #loss가 작은 걸 기준으로 mse(평균 제곱 오차) 잡는다 / 최적화 adam을 사용한다  / 최적의 weight를 구할 것임(=최소의 loss값)
model.fit(x, y, epochs= 1000, batch_size=1)  #정제된 데이터를 훈련시킨다. / epochs : 훈련을 1000번 시킨다. / batch_size : 몇 단 씩 잘라서 훈련시킬 것인가.
...
#result = model.predict([4]) #predic 안에는 예측하고 싶은 값을 넣는다.
print('result : ', result)
