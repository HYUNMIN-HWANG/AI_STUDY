...
pred_generator = test_datagen.flow(x_pred)
...
path = '../data/LPD_competition/cp/cp_0318_4_resnet_{accuracy:.4f}.hdf5'
cp = ModelCheckpoint(path, monitor='val_loss', save_best_only=True, mode='min')

model.compile(loss='categorical_crossentropy', optimizer=Adam(lr=1e-5), metrics=['accuracy'])
model.fit_generator(train_generator, epochs=500, steps_per_epoch = len(x_train) // batch ,
...
model = load_model('../data/LPD_competition/cp/cp_0318_4_resnet_last.hdf5')
model.summary()
