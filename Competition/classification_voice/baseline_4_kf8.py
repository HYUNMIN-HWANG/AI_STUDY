...
train_y = np.concatenate((np.zeros(len(africa_train_data[:500]), dtype = np.int),
                        np.ones(len(australia_train_data[:500]), dtype = np.int),
                         np.ones(len(canada_train_data[:500]), dtype = np.int) * 2,
                         np.ones(len(england_train_data[:500]), dtype = np.int) * 3,
                         np.ones(len(hongkong_train_data[:500]), dtype = np.int) * 4,
                         np.ones(len(us_train_data[:500]), dtype = np.int) * 5), axis = 0)
# print(train_x.shape, train_y.shape, test_x.shape)   # (25520, 64, 501, 1) (25520,) (6100, 64, 501, 1)
print(train_x.shape, train_y.shape, test_x.shape)   # (3000, 64, 501, 1) (3000,) (6100, 64, 501, 1)
...
cp_path = 'E:\\data\\classification_voice\\cp\\baseline_4.h5'
...
for col in range(1,7) :
    ...
    sample_submission.to_csv(f"E:\\data\\classification_voice\\baseline_0520_mean3.csv", index=False)
    print("=============")
