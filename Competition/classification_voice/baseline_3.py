...
cp_path = 'E:\\data\\classification_voice\\cp\\baseline_3.h5'
...
for train_idx, val_idx in split.split(train_x, train_y):
    ...
    # history = model.fit(x = x_train, y = y_train, validation_data = (x_val, y_val), epochs = 12, callbacks=[es, rl, cp])
    history = model.fit(x = x_train, y = y_train, validation_data = (x_val, y_val), epochs = 1, callbacks=[es, rl, cp])
    print("*******************************************************************")
...
for col in range(1,7) :
    ...
    sample_submission.to_csv(f"E:\\data\\classification_voice\\baseline_0520_mean2.csv", index=False)
    print("=============")
