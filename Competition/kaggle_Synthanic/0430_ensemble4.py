...
label_cols = ['Ticket', 'Sex']        # 'Pclass'를 label columns로 이동 
onehot_cols = ['Pclass', 'Cabin', 'Embarked']
...
print (f'X_train:{X_train.shape} y_train: {y_train.shape}') # X_train:(160000, 20) y_train: (160000,)
print (f'X_test:{X_test.shape} y_test: {y_test.shape}') # X_test:(40000, 20) y_test: (40000,)

test = all_df[len(train_df):].drop([TARGET], axis = 1)
print (f'test:{test.shape}')    # test:(100000, 20)
...
print(f"Current threshold is: {threshold}") #   0.2758909437633077
submission['submit_1'] = (test_preds > threshold).astype(int)
submission['submit_2'] = pd.read_csv("E:\\data\\kaggle_tabular\\pseudo_label.csv")[TARGET]
...
submission[['PassengerId', TARGET]].to_csv("E:\\data\\kaggle_tabular\\submission_0430_4.csv", index = False)

# submission_0430_4.csv 
# score 0.81710
