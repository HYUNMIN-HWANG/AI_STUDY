...
def split_dataset(path: os.PathLike) -> None:
    ...
    df.to_csv('../data/DACON_vision2/split_kfold.csv', index=False)
...
def train(fold: int, verbose: int = 30) -> None:
    split_dataset('../data/DACON_vision2/dirty_mnist_2nd_answer.csv')
    df = pd.read_csv('../data/DACON_vision2/split_kfold.csv')
    df_train = df[df['kfold'] != fold].reset_index(drop=True)
    df_valid = df[df['kfold'] == fold].reset_index(drop=True)

    df_train.drop(['kfold'], axis=1).to_csv(f'../data/DACON_vision2/train-kfold-{fold}.csv', index=False)
    df_valid.drop(['kfold'], axis=1).to_csv(f'../data/DACON_vision2/valid-kfold-{fold}.csv', index=False)

    trainset = MnistDataset('data/train', f'../data/DACON_vision2/train-kfold-{fold}.csv', transforms_train)
    train_loader = DataLoader(trainset, batch_size=args.batch_size, shuffle=True, num_workers=12)

    validset = MnistDataset('data/train', f'../data/DACON_vision2/valid-kfold-{fold}.csv', transforms_test)
...
def test(device: torch.device = 'cuda'):
    submit = pd.read_csv('../data/DACON_vision2/sample_submission.csv')
    ...
    submit.to_csv('../data/DACON_vision2/resnet101-e50-kfold.csv', index=False)
