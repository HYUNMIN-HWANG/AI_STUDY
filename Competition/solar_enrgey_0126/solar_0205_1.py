...
#2. Modeling
def modeling (drop=0.5, optimizer='adam', node=16, activation='relu', kernel_size=2, lr = 0.01) :
    model = Sequential()
    model.add(Conv1D(filters=16, kernel_size=kernel_size, activation='relu', padding='same',\
         input_shape=(x_train.shape[1], x_train.shape[2]))) # input (N, 5, 6)
    model.add(Conv1D(filters=16, kernel_size=kernel_size, activation=activation, padding='same'))
    model.add(Conv1D(filters=32, kernel_size=kernel_size, activation=activation, padding='same'))
    model.add(Conv1D(filters=32, kernel_size=kernel_size, activation=activation, padding='same'))
    model.add(Conv1D(filters=64, kernel_size=kernel_size, activation=activation, padding='same'))
    model.add(Conv1D(filters=64, kernel_size=kernel_size, activation=activation, padding='same'))
    model.add(Conv1D(filters=128, kernel_size=kernel_size, activation=activation, padding='same'))
    model.add(Conv1D(filters=128, kernel_size=kernel_size, activation=activation, padding='same'))
    model.add(Conv1D(filters=256, kernel_size=kernel_size, activation=activation, padding='same'))
    model.add(Conv1D(filters=256, kernel_size=kernel_size, activation=activation, padding='same'))
...
model2 = KerasRegressor(build_fn=modeling, verbose=1) 
...
for q in quantiles :
    ...
    cp_save = f'../data/modelcheckpoint/solar_0208_s1_q_{q:.1f}.hdf5'
    ...
    # 4. Evaluate, Predict
    loss = model.score(x_test, y_test, batch_size=8)
    print("loss : ", loss)
    ...
    submission.to_csv(f'../data/DACON_0126/submission_0208_1_{q}.csv', index = False)  # score : 
...
# to csv
submission.to_csv('../data/DACON_0126/submission_0208_1.csv', index=False)  # score : 
