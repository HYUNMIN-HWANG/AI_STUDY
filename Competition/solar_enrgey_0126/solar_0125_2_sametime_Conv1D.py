# 같은 시간대 별로 묶는다.
# ['T-Td'] 컬럼 추가, 시간 컬럼을 제외한 모든 컬럼 사용
...
def modeling() :
    ...
    model.add(Conv1D(filters=128, kernel_size=2, activation='relu', padding='same',\
         input_shape=(x_train.shape[1], x_train.shape[2]))) # input (N, 5, 9)
    model.add(Conv1D(filters=128, kernel_size=2, activation='relu', padding='same'))
...
for q in quantiles :
    ...
    cp_save = f'../data/modelcheckpoint/solar_0125_s2_q_{q:.1f}.hdf5'
    ...
    lr = ReduceLROnPlateau(monitor='val_loss', patience=4, factor=0.6, verbose=2)

    hist = model.fit(x_train, y_train, epochs = epoch, batch_size = batch, validation_data=(x_val, y_val), callbacks=[cp, lr])

    # 4. Evaluate, Predict
    loss = model.evaluate(x_test, y_test,batch_size = batch)
    print("loss : ", loss)
    ...
    submission.to_csv(f'../data/DACON_0126/submission_0125_2_{q}.csv', index = False) 
...
# to csv
submission.to_csv('../data/DACON_0126/submission_0125_2.csv', index=False)  # score : 		


# loss_mean :  4.748979727427165
# 9 loss :  [2.055678606033325, 2.25107479095459, 2.412372589111328, 8.222723960876465, 10.45783805847168, 2.408858060836792, 12.604239463806152, 1.5084705352783203, 0.8195614814758301]