# 코랩으로 돌렸음 (경로 다름)
# pca (cumsum > 0.999) : 468열로 압축
# 결과 너~~~~~~~무 안 좋음 (score 0.2058823529) 
...
print(x2.shape)      # (2048, 468)
print(x2_pred.shape) # (20480, 468)
...
x2 = x2.reshape(-1, 18 , 26 ,1)
x2_pred = x2_pred.reshape(-1, 18, 26, 1)

print(x2.shape)       # (2048, 18, 26, 1)
print(x2_pred.shape)  # (20480, 18, 26, 1)
...
for train_index, test_index in skf.split(x2, y) : # >>> x, y
    path = '/content/drive/My Drive/AIA/DACON_vision1/cp/0203_3_cp.hdf5'
    ...
    print(x_train.shape, x_test.shape, x_valid.shape)  # (1880, 18, 26, 1) (69, 18, 26, 1) (99, 18, 26, 1)
    print(y_train.shape, y_test.shape, y_valid.shape)  # (1880, 10) (69, 10) (99, 10)

    #2. Modeling
    model = Sequential()

    model.add(Conv2D(16, (3,3), activation='relu', input_shape=(18, 26,1), padding='same'))
    ...
    model.load_weights('/content/drive/My Drive/AIA/DACON_vision1/cp/0203_3_cp.hdf5')
    ...
    print(val_loss_min, np.mean(val_loss_min))  # val_loss_min 2.2582472562789917
    model.summary()

sub['digit'] = result.argmax(1)
print(sub)
sub.to_csv('/content/drive/My Drive/AIA/DACON_vision1/0203_3_private3.csv', index=False)

# xian submission 0203_2_pca.csv
# score 0.2058823529	