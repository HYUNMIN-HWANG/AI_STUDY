# private 2등 코드
# 노이즈 제거
# fastNlMeansDenoisingColored(s, None, 15, 15, 5, 10) 모델 돌려보기
...
for i, s in enumerate(x_train):
    # print(i)        # 0
    # print(s)    
    # print(s.shape)  # (28, 28, 1)
    train_224 = cv2.fastNlMeansDenoisingColored(s, None, 15, 15, 5, 10)
    # cv2.imshow('blur', train_224)
    cv2.waitKey(0)
...
for i, s in enumerate(x_test):
    test_224 = cv2.fastNlMeansDenoisingColored(s, None, 15, 15, 5, 10)
    # cv2.imshow('blur', test_224)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
...
train_224 = train_224.reshape(-1, 28, 28, 1)

for train_index, test_index in skf.split(train_224, y) : # >>> x, y
    path = '../data/DACON_vision1/cp/0204_4_cp.hdf5'
    ...
    model.load_weights('../data/DACON_vision1/cp/0204_4_cp.hdf5')
...
print(sub)
sub.to_csv('../data/DACON_vision1/0204_4_sub.csv', index=False)
