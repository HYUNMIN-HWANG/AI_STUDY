...
# 원하는 열만 가져오기
dataset = pd.read_csv('../data/DACON_0126/train/train.csv', index_col=None, header=0)
print(dataset.shape)

x_train = dataset.iloc[:,[1,3,4,5,6,7,8]]
print(x_train.columns)  # Index(['Hour', 'DHI', 'DNI', 'WS', 'RH', 'T', 'TARGET'], dtype='object')
...
subfile = pd.read_csv('../data/DACON_0126/sample_submission.csv')


def mymodel():
    model = Sequential()
    model.add(Conv2D(96, (1,2), \
        input_shape=(x_train.shape[1], x_train.shape[2], x_train.shape[3]), padding='same', activation='relu'))
    ...
    # model = Sequential()
    # model.add(Conv1D(filters=96, kernel_size=2, activation='relu', padding='same',\
...
for q in qlist:
    ...
    filepath = f'../data/modelcheckpoint/solar_0122_{q:.1f}.hdf5'
    check = ModelCheckpoint(filepath = filepath, monitor = 'val_loss', save_best_only=True, mode='min') #앞에 f를 붙여준 이유: {}안에 변수를 넣어주겠다는 의미
    hist = model.fit(x_train, y_train, epochs=10, batch_size=48, verbose=1, validation_split=0.2, callbacks=[stop, reduce_lr])#, check])
...
subfile.to_csv('../data/DACON_0126/submission_youngli.csv', index=False)
