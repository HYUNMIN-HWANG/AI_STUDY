# https://github.com/dongjaeseo/study/blob/main/practice/dacon_03_0120_tryMinMax.py
...
for i in quantiles:
    model = mymodel()
    filepath_cp = f'../data/modelcheckpoint/dacon_02_y1_quantile_{i:.1f}.hdf5'
...
for i in quantiles:
    model = mymodel()
    filepath_cp = f'../data/modelcheckpoint/dacon_02_y2_quantile_{i:.1f}.hdf5'
    cp = ModelCheckpoint(filepath_cp,save_best_only=True,monitor = 'val_loss')
    model.compile(loss = lambda y_true,y_pred: quantile_loss(i,y_true,y_pred), optimizer = 'adam', metrics = [lambda y,y_pred: quantile_loss(i,y,y_pred)])
...
submission.to_csv('../data/DACON_0126/0120_trynew.csv', index = False)
