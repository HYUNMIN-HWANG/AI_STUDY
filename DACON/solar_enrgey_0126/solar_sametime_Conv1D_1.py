...
def modeling() :
    model = Sequential()
    model.add(Conv1D(filters=125, kernel_size=2, activation='relu', padding='same',\
         input_shape=(x_train.shape[1], x_train.shape[2]))) # input (N, 5, 6)
    model.add(Conv1D(filters=50, kernel_size=2, activation='relu', padding='same'))
    model.add(Conv1D(filters=25, kernel_size=2, activation='relu', padding='same'))
    model.add(Conv1D(filters=25, kernel_size=2, activation='relu', padding='same'))
...
for q in quantiles :
    ...
    cp_save = f'../data/cp/solar_0123_s_q_{q:.1f}.hdf5'
    ...
    lr = ReduceLROnPlateau(monitor='val_loss', patience=12, factor=0.6, verbose=1)

    model.fit(x_train, y_train, epochs=10, batch_size=25, validation_data=(x_val, y_val), callbacks=[es, cp, lr])

    # 4. Evaluate, Predict
    loss = model.evaluate(x_test, y_test,batch_size=25)
    print("loss : ", loss)
    ...
    submission.to_csv(f'../data/submission_0123_3_{q}.csv', index = False)  # score : 
...
# to csv
submission.to_csv('../data/submission_0123_3.csv', index=False)  # score : 		2.1841329701	
