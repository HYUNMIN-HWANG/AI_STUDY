...
for q in quantiles :
    ...
    cp_save = f'../data/modelcheckpoint/solar_0124_s3_q_{q:.1f}.hdf5'
    ...
    hist = model.fit(x_train, y_train, epochs=10, batch_size=8, validation_data=(x_val, y_val), callbacks=[cp, lr])
    ...
    submission.to_csv(f'../data/DACON_0126/submission_0124_3_{q}.csv', index = False)  # 
...
# to csv
submission.to_csv('../data/DACON_0126/submission_0124_3.csv', index=False)  # score : 2.2447834533	

# 그래프 모양은 진짜 예쁘게 나왔는데 점수는 별로다? ? 그래프 모양이랑은 상관없는건가
